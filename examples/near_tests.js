...
function test_promise_batch_action_add_key_with_full_access() {
    let promise_id = env.promise_batch_create('john')
    env.promise_batch_action_add_key_with_full_access(promise_id, 'ed25519:5do5nkAEVhL8iteDvXNgxi4pWK78Y7DDadX11ArFNyrf', 0)
}

function test_promise_batch_action_add_key_with_function_call() {
    let promise_id = env.promise_batch_create('john')
...
function test_promise_return() {
    let promise_id = env.promise_batch_create('john')
    env.promise_return(promise_id)
}

function test_storage_access() {
    let key1 = 'key1'
    let value1 = 'value1'
...
function test_validator_stake() {
    if (env.validator_stake('bob') != 0) {
        env.panic()
    }
}

function test_validator_total_stake() {
    if (env.validator_total_stake() != 0) {
        env.panic()
    }
}

function test_alt_bn128_g1_multiexp() {
...
function test_alt_bn128_g1_sum() {
...
function test_alt_bn128_pairing_check() {
