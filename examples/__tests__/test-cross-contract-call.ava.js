...
test.beforeEach(async t => {
    ...
    const onCall = await root.createAndDeploy(
        root.getSubAccount('cross-contract-call').accountId,
...
test('Person can be set on-call if AVAILABLE', async t => {
    const { ali, bob, onCall } = t.context.accounts;

    // Ali set her status as AVAILABLE
    await ali.call(onCall, 'set_status', { message: 'AVAILABLE' });
...
test('Person can NOT be set on-call if UNAVAILABLE', async t => {
    const { ali, bob, onCall, statusMessageContract, onCallContract } = t.context.accounts;

    // Ali set her status as AVAILABLE
    await ali.call(onCall, 'set_status', { message: 'UNAVAILABLE' });
