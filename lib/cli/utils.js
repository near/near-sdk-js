...
export async function validateContract(contractPath) {
    ...
    for (const contractClass of contractClasses) {
        ...
        if (hasBindgen) {
            ...
            if (!hasConstructor && propertiesToBeInited.length > 0) {
                console.log(chalk.redBright(`Ops, constructor is not initialized, after initialization include ${propertiesToBeInited
                    .map((p) => p.name)
                    .join(", ")} in constructor`));
                process.exit(2);
            }
            ...
            if (nonInitedProperties.length > 0) {
                console.log(chalk.redBright(`Ops, please initialise ${nonInitedProperties.join(", ")} in constructor`));
                process.exit(2);
            }
        }
    }
