...
if (PLATFORM !== "win32") {
    ...
    const BINARYEN_SYSTEM_NAME = PLATFORM === "linux" ? "Linux" :
        PLATFORM === "darwin" ? "macOS" : "other";
    const BINARYEN_ARCH_NAME = ARCH === "x64" ? "X64" :
...
const QUICK_JS_SYSTEM_NAME = PLATFORM === "linux" ? "Linux" :
    PLATFORM === "darwin" ? "macOS" :
        PLATFORM === "win32" ? "win32" : "other";
const QUICK_JS_ARCH_NAME = ARCH === "x64" ? "X64" :
    ARCH === "arm64" ? "arm64" : "other";
...
// Download QuickJS
await download(`https://github.com/near/quickjs/releases/download/${QUICK_JS_VERSION_TAG}/${QUICK_JS_DOWNLOADED_NAME}`);
...
const WASI_SDK_SYSTEM_NAME = PLATFORM === "linux" ? "linux" :
    PLATFORM === "darwin" ? "macos" :
