...
export class Vector {
    ...
    swapRemove(index, options) {
        ...
        assert(near.storageWrite(bytes(key), serializeValueWithOptions(last, options)), ERR_INCONSISTENT_STATE);
    ...
    replace(index, element, options) {
        assert(index < this.length, ERR_INDEX_OUT_OF_BOUNDS);
        const key = indexToKey(this.prefix, index);
        assert(near.storageWrite(bytes(key), serializeValueWithOptions(element, options)), ERR_INCONSISTENT_STATE);
        const value = near.storageGetEvicted();
        return getValueWithOptions(value, options);
    }
    /**
