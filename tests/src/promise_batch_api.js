...
export function test_promise_batch_stake() {
    ...
    near.promiseBatchActionStake(promiseId, 100000000000000000000000000000n, near.signerAccountPk())
...
export function test_promise_add_full_access_key() {
    ...
    near.promiseBatchActionAddKeyWithFullAccess(promiseId, near.signerAccountPk(), 1n)
    near.promiseReturn(promiseId)
}
...
export function test_promise_add_function_call_access_key() {
    ...
    near.promiseBatchActionCreateAccount(promiseId)
    near.promiseBatchActionTransfer(promiseId, 10000000000000000000000000n)
    near.promiseBatchActionAddKeyWithFunctionCall(promiseId, near.signerAccountPk(), 1n, 250000000000000000000000n, 'caller2.test.near', 'test_promise_batch_create_transfer')
    near.promiseReturn(promiseId)
}
...
export function test_promise_batch_deploy_call() {
    ...
    near.promiseBatchActionDeployContract(promiseId, includeBytes('../build/promise_api.wasm'))
    near.promiseBatchActionFunctionCall(promiseId, 'cross_contract_callee', bytes('abc'), 0, 2 * Math.pow(10, 13))
    near.promiseReturn(promiseId)
}
