...
test('payable: true functions works with deposit', async (t) => {
  ...
  await t.notThrowsAsync(
    ali.call(payableContract, 'setValueWithPayableFunction', { value: 'hello' }, { attachedDeposit: DEPOSIT })
  )
})
...
test('payable: false throws if atach deposit', async (t) => {
  ...
  t.assert(
    result.result.status.Failure.ActionError.kind.FunctionCallError.ExecutionError.includes('Function is not payable')
  )
})
