...
test('add secp256k1 key string should success', async t => {
    const { bob, pkContract } = t.context.accounts;
    let r = await bob.callRaw(pkContract, 'test_add_secp256k1_key_string', '', {gas: '100 Tgas'});
    t.is(r.result.status.SuccessValue, '');
});
...
test('add invalid key should error', async t => {
    ...
    t.is(r.result.status.Failure.ActionError.kind.FunctionCallError.ExecutionError, 'VM Logic provided an invalid public key');
});
...
test('create invalid curve type should fail', async t => {
    ...
    t.is(r.result.status.SuccessValue, undefined);
    t.assert(r.result.status.Failure.ActionError.kind.FunctionCallError.ExecutionError.startsWith('Smart contract panicked: Unknown curve'));
})
...
test('create invalid length should fail', async t => {
    ...
    t.is(r.result.status.SuccessValue, undefined);
    t.assert(r.result.status.Failure.ActionError.kind.FunctionCallError.ExecutionError.startsWith('Smart contract panicked: Invalid length'));
})
...
test('create invalid base58 should fail', async t => {
    ...
    t.is(r.result.status.SuccessValue, undefined);
    t.assert(r.result.status.Failure.ActionError.kind.FunctionCallError.ExecutionError.startsWith('Smart contract panicked: Base58 error'));
})