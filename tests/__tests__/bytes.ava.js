...
test('storage write bytes tests', async (t) => {
  ...
  t.deepEqual(stateMap.get(encodeStateKey('abc')), Buffer.from('def').toString('base64'))
  t.deepEqual(stateMap.get(encodeStateKey([0x00, 0x01, 0xff])), Buffer.from([0xe6, 0xb0, 0xb4]).toString('base64'))
  t.deepEqual(
...
test('storage write unexpected types tests', async (t) => {
  ...
  t.deepEqual(stateMap.get(encodeStateKey('123')), Buffer.from('456').toString('base64'))
