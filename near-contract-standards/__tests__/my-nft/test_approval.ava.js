...
test("Simple approve", async (t) => {
  ...
  let alice_approval_id_is_1 = await nft.view('nft_is_approved', ['0', ali.accountId, '1'])
  t.assert(alice_approval_id_is_1)

  let alice_approval_id_is_2 = await nft.view('nft_is_approved', ['0', ali.accountId, '2'])
  t.assert(!alice_approval_id_is_2)

  res = await nftOwner.callRaw(nft, 'nft_approve', ['0', ali.accountId, null], {attachedDeposit: '1'})
  t.is(res.result.status.SuccessValue, "");
  alice_approval_id_is_2 = await nft.view('nft_is_approved', ['0', ali.accountId, '2'])
  t.assert(alice_approval_id_is_2)

  res = await nftOwner.callRaw(nft, 'nft_approve', ['0', bob.accountId, null], {attachedDeposit: '450000000000000000000'})
  t.is(res.result.status.SuccessValue, "");

  let bob_approval_id_is_3 = await nft.view('nft_is_approved', ['0', bob.accountId, '3'])
  t.assert(bob_approval_id_is_3)
});


test('Approve call', async (t) => {
  const { ali, bob, nft, nftOwner, approvalReceiver } = t.context.accounts;

  let res = await nftOwner.call(nft, 'nft_approve', ['0', approvalReceiver.accountId, 'return-now'], {attachedDeposit: '450000000000000000000', gas: '300 Tgas'})
  t.is(res, "cool");

  res = await nftOwner.call(nft, 'nft_approve', ['0', approvalReceiver.accountId, 'hahaha'], {attachedDeposit: '1', gas: '300 Tgas'})
  t.is(res, "hahaha");
})
...
test('Approved account transfers token', async (t) => {
  ...
  res = await ali.callRaw(nft, 'nft_transfer', [ali.accountId, '0', null, 'gotcha! bahahaha'], {attachedDeposit: '1'})
  t.is(res.result.status.SuccessValue, "");
...
test('revoke', async (t) => {
  ...
  res = await nftOwner.callRaw(nft, 'nft_approve', ['0', bob.accountId, null], {attachedDeposit: '510000000000000000000'})
  t.is(res.result.status.SuccessValue, "");

  res = await nftOwner.callRaw(nft, 'nft_revoke', ['0', ali.accountId], {attachedDeposit: '1'})
  t.is(res.result.status.SuccessValue, "");

  let alice_approved = await nft.view('nft_is_approved', ['0', ali.accountId, null])
  t.assert(!alice_approved)

  let bob_approved = await nft.view('nft_is_approved', ['0', bob.accountId, null])
  t.assert(bob_approved)

  res = await nftOwner.callRaw(nft, 'nft_revoke', ['0', bob.accountId], {attachedDeposit: '1'})
  t.is(res.result.status.SuccessValue, "");

  alice_approved = await nft.view('nft_is_approved', ['0', ali.accountId, null])
  t.assert(!alice_approved)
...
test('revoke all', async (t) => {
  ...
  res = await nftOwner.callRaw(nft, 'nft_revoke_all', '0', {attachedDeposit: '1'})
  t.is(res.result.status.SuccessValue, "");
